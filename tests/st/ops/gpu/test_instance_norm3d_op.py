...
class InstanceNorm3dNet(nn.Cell):
    def __init__(self, is_training=False, epsilon=1e-5, momentum=0.9):
        super(InstanceNorm3dNet, self).__init__()
        self.norm = P.InstanceNorm3D(is_training=is_training, epsilon=epsilon, momentum=momentum)

    def construct(self, x, gamma, beta, mean, variance):
        return self.norm(x, gamma, beta, mean, variance)


def LayerNormReference(epsilon, momentum, x, gamma, beta, mean, variance):
