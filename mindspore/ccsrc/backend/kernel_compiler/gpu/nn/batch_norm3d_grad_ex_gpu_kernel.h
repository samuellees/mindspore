...
#ifndef MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BATCH_NORM3D_GRAD_EX_GPU_KERNEL_H_
#define MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BATCH_NORM3D_GRAD_EX_GPU_KERNEL_H_
...
namespace kernel {
template <typename T>
class BatchNorm3dGradExGpuKernel : public GpuKernel {
 public:
  BatchNorm3dGradExGpuKernel()
      : x_size_(0),
        para_size_(0),
        workspace_size_(0),
        reserve_size_(0),
        ...
        beta_data_diff_(0) {}
  ~BatchNorm3dGradExGpuKernel() override { DestroyResource(); }
  ...
  bool Init(const CNodePtr &kernel_node) override {
    ...
    if (kernel_name != "BatchNorm3DGradEx") {
    ...
    if (shape.size() != 5) {
      MS_LOG(EXCEPTION) << "tensor shape is " << shape.size() << ", BatchNorm3dGradExGpuKernel should be 5";
    }
    is_null_input_ = CHECK_NULL_INPUT(shape);
    if (is_null_input_) {
      MS_LOG(WARNING) << "BatchNorm3dGradExGpuKernel input is null";
      InitSizeLists();
      return true;
    }
 ...
 private:
  void SetTensorDescriptor(const std::string &format, const std::vector<size_t> &shape) {
    ...
    if (bn_ops_ != CUDNN_BATCHNORM_OPS_BN) {
      CHECK_CUDNN_RET_WITH_EXCEPT(
        kernel_node_,
        cudnnSetTensorNdDescriptor(y_desc_, cudnn_data_type_, nbDims, dimA, strideAin),
        "Set z desc failed");
    }

    CHECK_CUDNN_RET_WITH_EXCEPT(
      kernel_node_, cudnnSetTensorNdDescriptor(dy_desc_, cudnn_data_type_, nbDims, dimA, strideAin),
      "Set dy desc failed");

    CHECK_CUDNN_RET_WITH_EXCEPT(
      kernel_node_, cudnnSetTensorNdDescriptor(dx_desc_, cudnn_data_type_, nbDims, dimA, strideAin),
      "Set dx desc failed");

    if (bn_ops_ == CUDNN_BATCHNORM_OPS_BN_ADD_ACTIVATION) {
      CHECK_CUDNN_RET_WITH_EXCEPT(
        kernel_node_,
        cudnnSetTensorNdDescriptor(dz_desc_, cudnn_data_type_, nbDims, dimA, strideAin),
        "Set z desc failed");
    }

    CHECK_CUDNN_RET_WITH_EXCEPT(
      kernel_node_,
      cudnnSetTensorNdDescriptor(scale_bias_diff_desc_, CUDNN_DATA_FLOAT, nbDims, dimAbias, strideAbias),
...
#endif  // MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BATCH_NORM3D_GRAD_EX_GPU_KERNEL_H_
