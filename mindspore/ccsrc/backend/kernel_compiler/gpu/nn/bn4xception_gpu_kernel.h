...
#ifndef MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BN4XCEPTION_GPU_KERNEL_H_
#define MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BN4XCEPTION_GPU_KERNEL_H_
...
namespace kernel {
template <typename T>
class BN4XceptionGpuKernel : public GpuKernel {
 public:
  BN4XceptionGpuKernel()
      : batch_(0),
        channel_(0),
        height_(0),
        width_(0),
  ...
  ~BN4XceptionGpuKernel() override { DestroyResource(); }
  ...
  bool Init(const CNodePtr &kernel_node) override {
    ...
    if (input_num != 5) {
      MS_LOG(EXCEPTION) << "input tensor size is " << input_num << ", BN4XceptionGpuKernel should be 5";
    }

    auto shape = AnfAlgo::GetPrevNodeOutputInferShape(kernel_node, 0);
    if (shape.size() != 4) {
      MS_LOG(EXCEPTION) << "tensor shape is " << shape.size() << ", BN4XceptionGpuKernel should be >= 4";
    }
    is_null_input_ = CHECK_NULL_INPUT(shape);
    if (is_null_input_) {
      MS_LOG(WARNING) << "BN4XceptionGpuKernel input is null";
      InitSizeLists();
      return true;
    }
...
#endif  // MINDSPORE_CCSRC_BACKEND_KERNEL_COMPILER_GPU_NN_BN4XCEPTION_GPU_KERNEL_H_
